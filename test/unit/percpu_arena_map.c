...
/*
 * percpu_arena_choose() used to compute the arena index from the CPU id with a
 * closed-form expression; it now reads a table built at boot.  These are the
 * pre-table expressions, kept here so the table is checked against them rather
 * than against a paraphrase of itself.
 */
static unsigned
legacy_ind_limit(percpu_arena_mode_t mode, unsigned ncpus_) {
...
static unsigned
legacy_choose(percpu_arena_mode_t mode, unsigned ncpus_, unsigned cpuid) {
...
TEST_BEGIN(test_map_matches_legacy) {
	...
	for (unsigned m = 0; m < sizeof(modes) / sizeof(modes[0]); m++) {
		...
		for (unsigned i = 0; i < NNCPUS; i++) {
			...
			expect_u_eq(ngroups, legacy_ind_limit(mode, n),
			    "Group count differs from the pre-table limit "
			    "(mode %s, ncpus %u)",
			    percpu_arena_mode_names[mode], n);

			for (unsigned cpu = 0; cpu < n; cpu++) {
				expect_u_eq(map[cpu], legacy_choose(mode, n, cpu),
				    "Map differs from the pre-table mapping "
