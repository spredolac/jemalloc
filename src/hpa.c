...
hpa_try_vectorized_purge(
  hpa_shard_t *shard, struct iovec *vec, size_t vlen, size_t nbytes) {
        bool success = opt_process_madvise_max_batch > 0
          && shard->central->hooks.vectorized_purge(vec, vlen, nbytes);
        if (!success) {
                /* On failure, it is safe to purge again (potential
                   perf penalty) If kernel can tell exactly which regions
...
hpa_try_purge(tsdn_t *tsdn, hpa_shard_t *shard) {
	...
	uint64_t purges_this_pass = 0;
        ...
        assert(opt_process_madvise_max_batch < 1024); // TODO: add IOV_MAX to configure
