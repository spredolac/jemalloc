...
static bool hpa_hooks_vectorized_purge(
  struct iovec *vec, size_t vlen, size_t nbytes);
...
/* Return true if we purged all nbytes, false otherwise */
static bool
hpa_hooks_vectorized_purge(struct iovec *vec, size_t vlen, size_t nbytes) {
